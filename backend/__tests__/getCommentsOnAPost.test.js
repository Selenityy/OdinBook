...
test("should get all comments on a post", async () => {
  const res = await request(app)
    .get(`/user/${actualUserId}/posts/${actualPostId}/comments`)
    .set("Authorization", `Bearer ${token}`);
  expect(res.statusCode).toEqual(200);
  console.log(res.body);
  const commentBodies = res.body.map((comment) => comment.body);
  expect(commentBodies).toContain("the first test comment");
  expect(commentBodies).toContain("the second test comment");
});
