...
beforeEach(async () => {
  ...
  await request(app).post(`/user/${userId}/sendFriendRequest/testuser2`);
});
...
test("should delete a user's own friend request", async () => {
  const res = await request(app)
    .post(`/user/${userId}/deleteFriendRequest/${friendUsername}`)
    .set("Authorization", `Bearer ${token}`);
  expect(res.statusCode).toEqual(200);
  expect(res.body.message).toEqual("Friend request deleted successfully");
